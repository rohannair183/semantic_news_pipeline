...
class Settings:
    ...
    def load_vector_search_config(
        ...
        """Load and validate typed vector search (bucket + index) configuration."""
        parser = (
            YAMLConfigParser()
            if configuration_root is None
            else YAMLConfigParser(configuration_root=configuration_root)
        ...
        raw = parser.parse(
            config_type=YAMLConfigType.SERVICE_LAYER,
            filename=filename,
        )
        section = cls._load_required_section(raw, section_name="vector_search")
        return cls._vector_search_from_mapping(section)

    @classmethod
    def _vector_search_from_mapping(cls, section: Dict[str, Any]) -> "VectorSearchConfig":
        """Parse ``vector_search`` YAML mapping into typed config."""
        bucket_name = cls._load_non_empty_string(
            section.get("bucket_name"),
            field_name="vector_search.bucket_name",
        )
        index_name = cls._load_non_empty_string(
            section.get("index_name"),
    ...
    @classmethod
    def load_briefing_generator_config(
        cls,
        configuration_root: Optional[Path] = None,
        filename: str = "briefing_generator.yaml",
    ) -> "BriefingGeneratorConfig":
        """Load and validate typed briefing generator (Gemini + topics) configuration."""
        parser = (
            YAMLConfigParser()
            if configuration_root is None
            else YAMLConfigParser(configuration_root=configuration_root)
        ...
        section = cls._load_required_section(raw, section_name="briefing_generator")
        return cls._briefing_generator_from_mapping(section)

    @classmethod
    def _briefing_generator_from_mapping(cls, section: Dict[str, Any]) -> "BriefingGeneratorConfig":
        """Parse ``briefing_generator`` YAML mapping into typed config."""
        model = cls._load_non_empty_string(
            section.get("model"),
            field_name="briefing_generator.model",
        )
        topics = cls._briefing_topics_from_yaml(section.get("topics"))
        raw_top_k = section.get("vector_top_k", 10)
        vector_top_k = cls._load_positive_int(
            raw_top_k,
            field_name="briefing_generator.vector_top_k",
        )
    ...
    def _briefing_topics_from_yaml(cls, value: Any) -> tuple["BriefingTopicSpec", ...]:
        ...
        if not isinstance(value, list) or not value:
            raise ValueError("briefing_generator.topics must be a non-empty list")
        specs: list[BriefingTopicSpec] = []
        for index, item in enumerate(value):
            prefix = f"briefing_generator.topics[{index}]"
            if not isinstance(item, dict):
                raise ValueError(f"{prefix} must be a mapping")
            name = cls._load_non_empty_string(
                item.get("name"),
                field_name=f"{prefix}.name",
            )
            vector_query = cls._load_non_empty_string(
...
class VectorSearchConfig:
    """Typed configuration for querying a Storage vector bucket index."""
...
class BriefingGeneratorConfig:
    """Typed configuration for Gemini briefing with vector-retrieved context."""
