...
def _is_retryable_supabase_api_exception(exc: BaseException) -> bool:
    ...
    if isinstance(exc, (ConnectionError, TimeoutError)):
        return True
    # Catch httpcore timeout exceptions (ReadTimeout, WriteTimeout, PoolTimeout)
    if type(exc).__name__ in ("ReadTimeout", "WriteTimeout", "PoolTimeout", "TimeoutException"):
        return True
    # Check for timeout strings in exception message (httpx client timeouts)
